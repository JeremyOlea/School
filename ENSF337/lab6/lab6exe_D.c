...
{
    char *holder = dest->dynamic_storage;
    char *new_space = malloc(dest->length*(int)sizeof('a')+source->length*(int)sizeof('a')+1);
    for(int i=0;i<dest->length;i++)
    {
        new_space[i] = dest->dynamic_storage[i];
    }
    for(int j=0; j<source->length; j++)
    {
        new_space[dest->length+j] = source->dynamic_storage[j];
    }
    new_space[dest->length+source->length+1] = '\0';
    dest->dynamic_storage = new_space;
...
{
    if(dest->length <new_length)
        return;

    char *holder = dest->dynamic_storage;
    dest->dynamic_storage = malloc(new_length*(int)sizeof('a')+1);
    for(int i=0; i<new_length;i++)
    {
        dest->dynamic_storage[i] = holder[i];
    }
