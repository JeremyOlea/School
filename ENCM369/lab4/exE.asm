...
#	bn		$t7	
	...
	addi $t0, $zero, '0'	# digit0 = '0'
	addi $t1, $zero, '1'	# digit1 = '1'
	addi $t2, $zero, '_'	# under = '_'
	add $t7, $zero, $zero	# bn = 0
	
	addi $t3, $a0, 39	# $t3 = str + 39
	sb $zero, ($t3)		# str[39] = \0
	addi $t4, $zero, 38	# index = 38
	addi $t5, $zero, 1	# mask = 1
	L1:
	and $t3, $a1, $t5	# $t3 = word & mask
	bne $t3, $zero, endIF1 
	add $t6, $a0, $t4	# $t6 = str + index
	sb $t0, ($t6)		# str[index] = digit0
	j endElse1
	endIF1:
	add $t6, $a0, $t4	# $t6 = str + index
	sb $t1, ($t6)		# str[index] = digit1 
	endElse1:
	
	addi $t4, $t4, -1	# index-- 
	addi $t7, $t7, 1	# bn++
	sll $t5, $t5, 1		# mask = mask << 1
